; (function ($, window, undefined) {
    ...
    $.fn.betterDropdown = function (options) {
       ...
       let option= $(settings.manualId+" option")
        let pgArray= $.map(option, function(x){
            return x.value;
        })
        ...
        if (settings.manualValueSet && settings.manualValue!=="" && settings.manualValue!==undefined) {
            selectItem($('.' + className + '-result').filter(function (e) {
                e+=1;
                return pgArray[e] === (settings.manualValue).toString();
            }));
        }
            },170)
